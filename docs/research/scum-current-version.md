...
이 문서는 SCUM 1.3.1.0.125621 (INTO THE WILD) 업데이트의 **프론트엔드 직결용 데이터 참고서**입니다. 각 정보는 출처 ID와 검증 상태를 기록하여 신뢰성을 추적합니다.

---

## 섹션 1: 공개 확인 테이블

현재 게임 버전의 기본 정보. 모두 Tier 2 (Steam 공식) 출처로 검증됨.
...
| # | 기능명 | 설명 | 검증상태 | 관련 도메인 | 기획 도메인 |
|---|-------|------|---------|-----------|-----------|
| 1 | 랜드 동물 추가 | 새로운 야생동물 종류 추가, 자유로운 배회 행동 패턴 | partial | hunting, enemies | `domains/scum/enemies/`, `domains/scum/hunting/` |
| 2 | 오픈월드 사냥 | 동물 서버 컨트롤 추가 (관리자 설정 가능) | verified | server-settings | `domains/scum/server-settings/` |
...
다음은 프론트엔드에서 검증 상태 배지와 함께 표시하기에 적합한 정보:
...
다음은 검증이 완료될 때까지 프론트엔드에서 **표시 금지** 또는 needs-review 태그 필수:
...
| `needs-review` | 주황색 느낌표 | 표시하되 "검증 중" 안내 + 재확인 날짜 표기 |
...
### 검증 메타데이터 (src/shared/types/index.ts)
...
**적용 방식**:
- 모든 게임 버전 정보는 GameVersion.meta를 통해 검증 상태 추적
- 개별 기능은 VersionFeature.status로 검증 상태 표시
- 프론트엔드는 meta.verificationStatus에 따라 배지 렌더링

---

## 섹션 6: 재검증 필요 항목 (조사 완료)
...
**다음 단계**: Bear/Boar/Wolf 완전 재검증 필요, Chicken/Goat/Donkey/Rabbit 세부사항 조사 필요
...
**다음 단계**: 각 즉흥 요리의 정확한 핵심 재료 조합 재검증 필요
...
| `implementation-plan.md` | 전체 Phase 계획 및 검증 스케줄 |
