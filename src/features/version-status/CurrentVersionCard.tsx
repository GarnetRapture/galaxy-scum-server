  ...
  if (error || !version) {
    return (
      <div className="scum-card" style={{ maxWidth: '600px', borderColor: 'rgba(255, 23, 68, 0.3)', backgroundColor: 'rgba(255, 23, 68, 0.05)' }}>
        <p style={{ color: 'var(--scum-red)', fontSize: '14px' }}>{t("version.loadError")}</p>
      </div>
    )
  }
  ...
  return (
    <div className="scum-card" style={{ maxWidth: '600px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--scum-cyan)', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t("version.versionInfo")}</h3>
        <dl style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '13px' }}>
          <div>
            <dt style={{ color: 'var(--scum-text-secondary)', fontSize: '12px', marginBottom: '4px' }}>{t("version.versionNumber")}</dt>
            <dd style={{ fontFamily: 'monospace', color: 'var(--scum-text-primary)', margin: 0 }}>{version.versionNumber}</dd>
          </div>
          <div>
            <dt style={{ color: 'var(--scum-text-secondary)', fontSize: '12px', marginBottom: '4px' }}>{t("version.releaseDate")}</dt>
            <dd style={{ color: 'var(--scum-text-primary)', margin: 0 }}>{version.releaseDate}</dd>
          </div>
          <div>
            <dt style={{ color: 'var(--scum-text-secondary)', fontSize: '12px', marginBottom: '4px' }}>{t("version.officialReleaseDate")}</dt>
            <dd style={{ color: 'var(--scum-text-primary)', margin: 0 }}>{version.officialReleaseDate || "—"}</dd>
          </div>
          <div>
            <dt style={{ color: 'var(--scum-text-secondary)', fontSize: '12px', marginBottom: '4px' }}>{t("version.verificationStatus")}</dt>
            <dd style={{ color: 'var(--scum-text-primary)', margin: 0, textTransform: 'capitalize' }}>
              {pickLocalizedText(VERIFICATION_STATUS_LABELS[version.meta.verificationStatus], locale)}
            </dd>
          </div>
        </dl>
      </div>
      ...
      <div>
        <h3 style={{ fontSize: '13px', fontWeight: 600, color: 'var(--scum-cyan)', marginBottom: '12px', marginTop: '16px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          {t("version.featureCount", undefined, { count: versionFeatures.length })}
        </h3>
        <div style={{ display: 'space-y-3' }}>
            ...
            <div key={feature.id} className="scum-card" style={{ marginBottom: '12px', padding: '12px' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                  <h4 style={{ fontWeight: 600, color: 'var(--scum-text-primary)', margin: 0, fontSize: '14px' }}>{pickLocalizedText(feature.title, locale)}</h4>
                  <VerificationBadge status={feature.status} size="sm" />
                </div>
                <p style={{ fontSize: '13px', color: 'var(--scum-text-secondary)', margin: 0 }}>{pickLocalizedText(feature.description, locale)}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ borderTop: '1px solid var(--scum-border)', paddingTop: '12px', marginTop: '16px', fontSize: '12px', color: 'var(--scum-text-muted)' }}>
        <p>
          {t("version.lastCheckedLine", undefined, { date: new Date(version.meta.checkedAt).toLocaleDateString(locale === "ko" ? "ko-KR" : "en-US") })}
          {version.meta.reviewBefore &&
            t("version.reviewByLine", undefined, { date: new Date(version.meta.reviewBefore).toLocaleDateString(locale === "ko" ? "ko-KR" : "en-US") })}
        </p>
      </div>
    </div>
  )
